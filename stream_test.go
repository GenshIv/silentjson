...
func TestStreamDecoder_Next(t *testing.T) {
	...
	if items[0].ID != 1 {
		t.Errorf("expected 1, got %d", items[0].ID)
	}
	if items[1].ID != 2 {
		t.Errorf("expected 2, got %d", items[1].ID)
	}
}
...
func TestStreamDecoder_NextRaw(t *testing.T) {
	...
	if string(raw1) != `{"id":1,"name":"Alice"}` {
		t.Errorf("got %s", raw1)
	}
...
func TestStreamDecoder_NextChan(t *testing.T) {
	...
	if items[2].ID != 3 {
		t.Errorf("expected 3, got %d", items[2].ID)
	}
}
